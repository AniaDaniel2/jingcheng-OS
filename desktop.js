...
let cms = {
    ...
    'desktop': [
        ...
        `<a onmousedown="window.open('https://github.com/AniaDaniel2/jingcheng-OS','_blank');" win12_title="https://github.com/AniaDaniel2/jingcheng-OS" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)"><i class="bi bi-github"></i> 在 Github 中查看此项目</a>`,
        function (arg) {
            if (edit_mode) {
        ...
        ['<i class="bi bi-info-circle"></i> 关于 jingcheng OS 网页版', `$('#win-about>.about').addClass('show');$('#win-about>.update').removeClass('show');openapp('about');if($('.window.about').hasClass('min'))minwin('about');`],
...
let nts = {
    'about': {
        cnt: `<p class="tit">jingcheng OS 网页版</p>
            <p>jingcheng OS 网页版是一个开放源项目,<br />
            希望让用户在网络上预先体验 jingcheng OS,<br />
            内容可能与 jingcheng OS 正式版本不一致。<br />
            使用标准网络技术,例如 HTML, CSS 和 JS<br />
            此项目绝不附属于微软,且不应与微软操作系统或产品混淆,<br />
            这也不是 jingcheng OS 365 cloud PC<br />
            本项目中微软、jingcheng OS和其他示范产品是微软公司的商标<br />
    ...
    'feedback': {
        cnt: `<p class="tit">反馈</p>
            <p>我们非常注重用户的体验与反馈</p>
            <list class="new">
                <a class="a" onclick="window.open('https://github.com/AniaDaniel2/jingcheng-OS','_blank');" win12_title="在浏览器新窗口打开链接" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)">在github上提交issue(需要github账户，会得到更高重视)</a>
    ...
    'about-copilot': {
        cnt: `
            <p class="tit">关于 jingcheng OS Copilot</p>
    ...
    'feedback-copilot': {
        cnt: `<p class="tit">反馈 jingcheng OS Copilot</p>
                ...
                res.json().then(json => {
                    ...
                    if (sha != json[0].sha) {
                        ...
                        if (msg.match(/v[0-9]*\.[0-9]*\.[0-9]*/)) {
                            ...
                            window.setTimeout(() => {
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .notice')[0].innerText = 'jingcheng OS 有更新可用';
                            ...
                            window.setTimeout(() => {
                                let da = new Date();
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .notice')[0].innerText = 'jingcheng OS 目前是最新版本';
                                ...
                                $('#win-setting>.page>.cnt.update>.setting-list>.update-now>div>p:last-child')[0].innerText = 'jingcheng OS 目前是最新版本';
                            }, 6000)
                        }
                    }
                });
...
let copilot = {
    ...
    init: () => {
        ...
        1.jingcheng OS 网页版是一个开源项目，由谭景元原创, 使用 Html,css,js，在网络上模拟、创新操作系统
            ...
            if (r.length) {
                for (const i of r) {
                    if (/{openapp .+?}/.test(i)) {
                        ...
                        shownotice('feedback');
                        rt = rt.replace(i, `<div class="action"><p class="tit">反馈</p><p class="detail">关于 jingcheng OS 网页版</p></div>`)
                    } else if (/{feedback copilot}/.test(i)) {
                        shownotice('feedback-copilot');
                        rt = rt.replace(i, `<div class="action"><p class="tit">反馈</p><p class="detail">关于 jingcheng OS Copilot</p></div>`)
